NETWORK_NAME="${NETWORK_NAME:-keycloak-net}"
...
if docker network inspect ${NETWORK_NAME} > /dev/null 2>&1 ; then
    echo -e "${GREEN}[+] => ${NETWORK_NAME} exist ${NC}"
else
    echo -e "${YELLOW}[+] => Creating ${NETWORK_NAME} network... ${NC}"
    ...
    eval "$NET_CMD"
    echo -e "${GREEN}[+] => ${NETWORK_NAME} network created successfully ${NC}"
fi
